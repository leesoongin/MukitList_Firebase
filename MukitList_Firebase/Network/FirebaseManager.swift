...
class FirebaseManager {
    ...
    func saveMukitList(id : String, document : Document){
        ...
        guard let key = db.child("MukitList").child("userid").childByAutoId().key else { return }
        let childUpdates = ["/MukitList/userid/\(key)": document.toDictionary]
        db.updateChildValues(childUpdates)
        
    }
    //먹킷리스트 불러오기
    func loadMukitList(id : String, completion : @escaping ([Document])->(Void)){
        db.child("MukitList").child("userid").observeSingleEvent(of: .value) { snapshot in
